...
`SkySim <SKYSIM_REPO_URL>`__ is an open-source multirotor flight simulator that
...
SkySim is distributed from its `repository <SKYSIM_REPO_URL>`__. Follow the
...
<SKYSIM_REPO_URL>`__. Problems with the simulator can be reported on its `issue
tracker <SKYSIM_REPO_URL/issues>`__.
